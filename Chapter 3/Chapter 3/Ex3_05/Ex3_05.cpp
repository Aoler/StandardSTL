...
int main()
{
  ...
  while (added++ < count)
  {
    ...
    (*iter)->add(std::unique_ptr<Customer>(new Customer(service_t)));
  ...
  while (time < total_time)                      // Simulation loops over time
  {
    ...
    if (--new_cust_interval == 0)
    {
      ...
      (*std::min_element(std::begin(checkouts), std::end(checkouts), comp))->add(std::unique_ptr<Customer>(new Customer(service_t)));
